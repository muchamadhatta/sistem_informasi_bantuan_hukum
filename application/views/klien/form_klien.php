		...
		<div class="row mb-2">
			...
			<div class="col-sm-6">
				<ol class="breadcrumb float-sm-right">
					<li class="breadcrumb-item"><a href="<?=site_url('klien')?>">Klien</a></li>
	...
	<div class="col col-md-12">
		<div class="card card-success">
			<div class="card-header">
				<h3 class="card-title"><?=ucfirst($page)?> Klien</h3>
			</div>
			<!-- /.card-header -->
			<!-- form start -->
			<?php echo form_open_multipart('klien/process') ?>
				<div class="card-body">
					<div class="form-group ">
					<input type="hidden" name="id_user" value="<?=$row->id_user?>">
					<input type="hidden" name="id_klien" value="<?=$row->id_klien?>">
					...
					<div class="form-group">
						...
						<input type="email" name="username" value="<?=$row->username?>" class="form-control"
							id="InputUsername" placeholder="Enter Username" >
						<?=form_error('username')?>
					</div>
					<div class="form-group">
						<label for="InputUsername">Tanggal Lahir</label>
						<input type="date" name="tanggal_lahir" value="<?=$row->tanggal_lahir?>" class="form-control" id="InputAlamat"
							placeholder="Input Tanggal Lahir">
						<?=form_error('tanggal_lahir')?>
					</div>
					<div class="form-group">
						<label for="InputUsername">Telepon</label>
						<input type="number" name="telepon" value="<?=$row->telepon?>" class="form-control" id="InputTelepon"
							placeholder="Input Telepon">
						<?=form_error('telepon')?>
					</div>
					<div class="form-group">
						<label for="InputUsername">Alamat</label>
						<input type="text" name="alamat" value="<?=$row->alamat?>" class="form-control" id="InputAlamat"
					...
					<div class="form-group">
						...
						<select name="level" class="form-control">
							<option value="klien" <?=$row->level == 'klien' ? "selected" : null?>>
								klien</option>
						</select>
						<?=form_error('level')?>
					</div>
