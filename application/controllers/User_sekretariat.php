...
class User_sekretariat extends CI_Controller {
	...
	public function index()
	{
		...
		$this->template->load('template', 'user_sekretariat/user_sekretariat', $data);
	}
	public function add()
	{
		...
		if ($this->form_validation->run() == FALSE)
                {
					$this->template->load('template', 'user_sekretariat/user_form_add');
				}
                else
                {
						...
						if($this->db->affected_rows() > 0) {
							...
							echo"<script>";
							echo"window.location='".site_url('user_sekretariat')."'";
							echo"</script>";
                }
	}
public function update($id)
		...
		if ($this->form_validation->run() == FALSE)
                {
					...
					if($query->num_rows() > 0) {
						$data['row'] = $query->row();
						$this->template->load('template', 'user_sekretariat/user_form_update', $data);
					} else {
							echo"<script>";
	...
	public function read($id){
		$query = $this->M_user_sekretariat->get($id);
					if($query->num_rows() > 0) {
						$data['row'] = $query->row();
						$this->template->load('template', 'user_sekretariat/user_read', $data);
					} else {
							echo"<script>";
							echo"alert('Data Tidak Ditemukan');";
							echo"window.location='".site_url('user_sekretariat')."'";
